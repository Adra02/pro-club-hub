...
// FIX: Usa l'URL corretto per il reset
const APP_URL = process.env.VERCEL_URL 
  ? `https://${process.env.VERCEL_URL}` 
  : process.env.NODE_ENV === 'production'
    ? 'https://pro-club-hub.vercel.app/'  // ⚠️ CAMBIA CON IL TUO URL VERCEL
    : 'http://localhost:3000';

export async function sendWelcomeEmail(email, name) {
  const transport = getTransporter();
  if (!transport) {
    console.log('Transporter not configured, skipping email');
    return false;
  }

  try {
    const info = await transport.sendMail({
      from: `"Pro Club Hub" <${process.env.GMAIL_EMAIL}>`,
      to: email,
